...
{
    if (!_leftButton) {
        _leftButton = [self buttonWithTitle:NSLocalizedString(@"取消", nil)];
...
{
    if (!_rightButton) {
        _rightButton = [self buttonWithTitle:NSLocalizedString(@"确定", nil)];
...
- (DZNPhotoEditorContainerView *)bottomView
{
    if (!_bottomView)
    {
        ...
        if (self.cropMode == DZNPhotoEditorViewControllerCropModeCircular)
        {
            ...
            topLabel.text = NSLocalizedString(@"", nil);
